...
public class VegetableTests {
    ...
    public void ValidRipeTest() {
        ...
        Assert.True(carrot.IsCollectable(new(2023, 3, 3)));
        Assert.True(carrot.IsCollectable(new(2023, 3, 4)));
        Assert.False(carrot.IsCollectable(new(2023, 3, 2)));

        carrot = new("Carrot", new(year: 2023, month: 2, day: 1), new(60), "Carrots are good for the eyes.", VegetableType.RootVegetables);

        Assert.True(carrot.IsCollectable(new(2023, 4, 2)));
        Assert.True(carrot.IsCollectable(new(2023, 5, 2)));
        Assert.False(carrot.IsCollectable(new(2023, 3, 2)));
    }
}
